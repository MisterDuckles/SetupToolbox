...
- **v0.8.8** — Out-of-scope leftover types die we voor v0.8.6/v0.8.7 hebben geparkeerd:
  - **Bredere registry scope** — momenteel alleen uninstall keys. CCleaner-niveau vereist:
    - **App Paths** (`HKLM/HKCU\Software\Microsoft\Windows\CurrentVersion\App Paths\<exe>`) → exe-pad bestaat niet meer = orphan
    - **MUIcache** (`HKCU\Software\Classes\Local Settings\Software\Microsoft\Windows\Shell\MuiCache`) → recently-used programma's met dode paden
    - **File extensions / ProgID classes** (`HKLM\Software\Classes\.<ext>` + `HKLM\Software\Classes\<ProgID>\shell\open\command`) → handler-exe weg = orphan
    - **HKCU\Software\\<Vendor>\\<App>** met `InstallPath` of `ExecutablePath` velden die niet bestaan → vendor-residue
    - Alle bestaande generieke filters (winget+AppX cross-check) hergebruiken
