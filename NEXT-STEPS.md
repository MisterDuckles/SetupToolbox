...
- **v0.8.7** — Orphaned registry uninstall keys: registry-leftovers van apps die ooit verwijderd zijn waarvan Windows de uninstall key niet opruimde, zelf orphan. `IsRegistryEntryAlive` helper checkt of tenminste één pad-veld (InstallLocation / UninstallString exe / DisplayIcon) feitelijk op disk bestaat — anders = orphan registry entry. Nieuwe `ScanOrphanedRegistryAsync` flow + nieuwe `OrphanedRegistry` enum value + `reg.exe delete /f` in DeleteAsync (HKCU user, HKLM elevated). Bonus: folder-scan gebruikt vanaf nu de **gefilterde** installed-list zodat folders die alleen via een dode registry-entry "matched" werden nu correct als orphan terugkomen. Bundle-by-name pakt registry+folder van zelfde app automatisch samen — `HKCU\...\Uninstall\Claude` + `AppData\Local\AnthropicClaude` in één bundle-card. Plan staat al uitgewerkt in chat-history
- **v0.8.8** — Out-of-scope leftover types die we voor v0.8.6/v0.8.7 hebben geparkeerd:
  - **Orphaned Start Menu shortcuts** — walk `%APPDATA%\Microsoft\Windows\Start Menu` + AllUsersProfile equivalent, parse `.lnk` files, target-pad bestaat niet meer = orphan shortcut
  - **Orphaned scheduled tasks** — enumerate via `Task Scheduler` API of `schtasks /Query /XML`, parse de `<Command>` paden, dood pad = orphan task
  - **Orphaned services** — enumerate via `Get-CimInstance Win32_Service`, check ImagePath bestaat, dood pad = orphan service. Risky (kan systeem breken) — strenger guards + duidelijke caution-tier
  - **Orphaned firewall rules** — enumerate via `Get-NetFirewallRule` / `Get-NetFirewallApplicationFilter`, check Program-pad bestaat. Lower priority — meeste users hebben hier weinig last van
