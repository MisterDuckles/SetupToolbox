...
- **v0.8.6** — Deep clean (los van uninstall): aparte sectie/dialog op DebloatPage voor system-wide cleanup, los van een specifieke uninstall. Twee categorieën:
  ...
  - **DebloatPage layout-restructure** — twee top-level secties als dropdowns (Expanders zoals v0.8.5 al toepast voor Microsoft / OEM): één **"Apps debloat"** groep met de bestaande Microsoft / OEM / All apps Expanders eronder, en één **"Deep clean"** groep voor de hierboven genoemde orphaned folders + Windows caches. User die alleen losse bestandsresten of mappen wil opruimen (zonder een app te uninstallen) gaat direct in Deep clean. **TBD**: precieze functionele scope van Deep clean — gaan we nog verder uitdenken (bv. custom folder pickers? scan op specifieke vendor-naam? kandidaten uit registry-only matches die geen app meer hebben?). Deze planning-stap zit zelf op de roadmap zodat we 'm niet vergeten
