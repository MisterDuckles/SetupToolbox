...
- **v0.8.9** — Restant out-of-scope leftover types die we niet in v0.8.8 meegenomen hebben:
  ...
  - **HKCU\Software\\<Vendor>\\<App>** met `InstallPath` of `ExecutablePath` velden die niet bestaan → vendor-residue. Generiek door alle `Software\<Vendor>\<App>` keys te walken en per key te checken op string-values die op pad lijken
