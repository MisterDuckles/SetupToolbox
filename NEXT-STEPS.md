...
Native Windows 11 app voor het bulk-installeren van apps via `winget`, plus debloat, tweaks en deep clean. **v2.0.1** (rebrand: heette voorheen *WingetAppDeployer*).
...
### Zonder versienummer — klein onderhoud, gevonden tijdens v2.0.1

- **De snapshot-backup dekt een pakketverwijder-tweak niet.** Gevonden in de reviewronde van v2.0.1 en bewust zo gelaten. `SnapshotService.CaptureAsync` bewaart registerwaarden; `Taskbar.HideWidgets` verwijdert sinds v2.0.1 een AppX-pakket en levert dus **nul entries** op. Zet je "backup vóór toepassen" aan en draai je die snapshot later terug, dan komt Widgets daar niet van terug — de tweak is wél omkeerbaar, maar via zijn eigen schakelaar. De belofte in de app ("momentopname van wat er gewijzigd wordt") klopt daarmee niet volledig voor deze ene tweak. Uitwegen: een entry-soort voor pakketten toevoegen (restore = dezelfde `Add-AppxPackage -Register`-stap die de revert al doet, dus de machinerie ligt er), of de backup-prompt laten zeggen wat er níét in zit. Het eerste is netter en waarschijnlijk een uurtje werk. De blinde vlek staat met deze reden ook in de code, en de website-FAQ is al wél eerlijk gemaakt.
...
- **`Selectie wissen` en `Clear all` beloven niet hetzelfde.** Ook gevonden op 2026-08-22 bij het naast elkaar leggen van de NL- en EN-schermafbeeldingen. De knop links van *Geselecteerde apps installeren* heet in het Nederlands *Selectie wissen* en in het Engels *Clear all*. Het eerste zegt "haal de vinkjes weg", het tweede zegt "gooi alles weg" — en op een pagina met een catalogus is dat een betekenisvol verschil. Zelfde soort afwijking als *Volledig actief* tegenover *Fully applied* (‘actief’ versus ‘toegepast’). Geen van beide is fout, maar ze vertellen een ander verhaal. Eén keuze maken en doorvoeren; het raakt alleen de waarde van een bestaande key, dus geen wees.

- **De schermafbeeldingen op de website hebben geen cachebrekende naam.** Vite hasht alleen wat het zelf uitstoot (`assets/index-*.js|css`); alles in `public/` houdt zijn naam. De deploy zet `screenshots/apps-nl.webp` met vaste naam over de bestaande heen, dus zodra een beeld ooit vervangen wordt blijven terugkerende bezoekers en elke tussenliggende cache het oude tonen. Uitwegen: de beelden via `src/` importeren zodat Vite ze hasht, of een versie-query achter de URL die je bij een nieuwe set ophoogt. Niet urgent — het speelt pas bij de twééde vervanging.

- **De hostkey van de deploy-server staat nog niet vast.** `deploy-website.yml` haalt ‘m nu bij elke run op met `ssh-keyscan` en vertrouwt wat er antwoordt. Dat is beter dan `StrictHostKeyChecking=no` (waar de bestaande workflow van BoG-Dashboard op staat), maar het blijft trust-on-first-use, en dit is **wachtwoord**-authenticatie — zit er iemand tussen de runner en de server, dan krijgt die het wachtwoord. Zodra de eerste run geslaagd is: de hostkey uit dat runner-log halen, één keer vergelijken met wat de hosting in het dashboard toont, en als secret `FTP_KNOWN_HOSTS` vastzetten. De workflow gebruikt die dan automatisch en de waarschuwing verdwijnt.
...
- ~~**De download-regel op de install-kaart toont de volledige URL en breekt over meerdere regels.**~~ — **opgelost in v1.2.9.3.** Niet met `Path.GetFileName` maar met `Uri.AbsolutePath`: die eerste knipt op de laatste slash en houdt een `?query` of `#fragment` vast, en op een URL zónder pad geeft ‘ie de host terug. Parst het niet als absolute URI — winget kan ook een lokaal pad of een los woord loggen — dan blijft de ruwe tekst staan. Oorspronkelijke tekst: Zichtbaar op de v1.2.8-screenshots: *"Downloading https://github.com/HandBrake/HandBrake/releases/download/1.11.2/HandBrake-1.11.2-x86_64-Win_GUI.exe"* loopt over twee regels en laat de kaart verspringen. De Nederlandse variant is nog iets langer. Bestond al vóór de vertaling en is bewust níét meegenomen in v1.2.8.1 (dat was een taalfix, geen layoutfix). Uitweg: alleen de bestandsnaam tonen in plaats van de hele URL — `install.winget.downloading` heeft al een `{0}`, dus het is één `Path.GetFileName`-aanroep.
...
- ~~**De filter-header in de Deep-clean-dialog toont het totaal in plaats van het aantal treffers.**~~ — **opgelost in v1.2.9.3.** De toewijzing stond vóór de filter berekend werd, dus er was op dat moment nog geen trefferaantal om mee te geven. Twee blokken omgedraaid; de header staat nu bewust ook vóór de lege-lijst-return, want anders blijft ‘ie op de telling van het vórige filter hangen. Oorspronkelijke tekst: Bij een actief filter staat er *"227 items gevonden — filter actief"* terwijl er 30 kaarten onder staan: `deepclean.foundFiltered` krijgt `_items.Count` mee in plaats van `filteredItems.Count`. Gezien tijdens de v1.2.7-verificatie, bestaat al sinds de filter er is en heeft niets met de vertaling te maken. Eénregelige fix.
...
### v2.0.1 — Drie tweaks die stil faalden, de Widgets-knop op 24H2+, en dialogs die het thema niet volgden

> **Uitgebracht op 2026-08-23 als GitHub Release, en dat is bewust een afwijking van de eigen regel.** De afspraak in `CLAUDE.md` is: Releases met exe alleen bij milestone-versies, patches niet. Hier is dat op verzoek van user losgelaten, met een reden die de regel niet ondermijnt: dit is een *reparatie* van tweaks die aantoonbaar niets deden — wachten tot de volgende milestone betekent dat iedereen tot dan met kapotte tweaks zit. De regel blijft dus staan voor gewone patches. Uitgeleverd: `SetupToolbox-v2.0.1.exe` (69,7 MB, naam matcht `^SetupToolbox-v.*\.exe$` zodat de self-update 'm ziet — gecontroleerd via `/releases/latest`), `FALLBACK_VERSION` op `v2.0.1`, en de website-deploy is meegelopen (Actions-run op commit `5464454`, geslaagd). De release-body is geschreven voor twee lezers tegelijk, conform de v1.2.10-afspraak: de GitHub-pagina én de wat-is-er-nieuw-popup die markdown als platte tekst rendert.

**Gemeld door user op 2026-08-23**, met een schermafbeelding van de Tweaks-pagina: *"12 wijzigingen mislukt — Access to the registry key 'HKEY_CURRENT_USER\Software\Policies\Microsoft\Office\16.0\…' is denied"*, plus de constatering dat *Widgets-knop verbergen* en *Spotlight-bureaubladachtergrond* niets doen, en dat de planning-dialog voor de auto-update donker blijft als je het Windows-thema wisselt terwijl de app open staat. De diagnose van user — "die hebben UAC nodig maar vragen er niet om" — klopte voor twee van de drie.

**1. `HKCU\Software\Policies` is voor de gebruiker zelf alleen-lezen.** Geverifieerd met `Get-Acl` op 25H2: de interactieve gebruiker heeft `ReadKey`, alleen `BUILTIN\Administrators` en `SYSTEM` hebben `FullControl` — Windows beschermt de user-policies zo tegen de user. Een geautomatiseerde scan over alle 56 HKCU-paden in `TweakService` vond zeven niet-schrijfbare paden, allemaal onder `Software\Policies`. Drie tweaks schreven daar in-process zonder `RequiresElevation` en faalden dus altijd, in beide richtingen: **Privacy.DisableOfficeTelemetry** (12 van de 13 ops; alleen `MailSettings` is een gewone user-key en slaagde — vandaar "12 mislukt, 31 wel"), **Ads.DisableDesktopSpotlight** en **NotifLock.DisableNotificationCenter**. De twee Start-tweaks op dezelfde Explorer-policy-key hadden de vlag al sinds 24H2.

- De drie tweaks hebben nu `RequiresElevation = true` (de Office-ops via een lokale `OfficePolicyOp`-helper); het admin-slotje verschijnt op hun kaart, en de omschrijvingen die "HKCU, geen UAC" beloofden zijn herschreven.
- **Vangnet zodat dit niet terugkomt:** `RequiresElevation` op `TweakOperation` en `TweakChoiceValue` is geen kale `init`-bool meer. Niet gezet, dan wordt 'ie afgeleid uit het pad via `RegistryPathRules.NeedsAdminToken` (HKLM, HKU, `HKCU\Software\Policies\*`, `HKCU\…\CurrentVersion\Policies\*`). Expliciet zetten blijft mogelijk en alle bestaande ops doen dat nog; het vangnet is er voor de volgende tweak die iemand vergeet te vlaggen.
- **De elevated batch adresseert HKCU nu als `HKU\<SID>`.** `reg.exe` draait onder het admin-token, en daar is "HKCU" het profiel van wie de UAC-prompt goedkeurde. Bij een split-token admin is dat dezelfde hive, maar een standaardgebruiker die het admin-account van iemand anders invult schreef de policy in het verkeerde profiel — en de detectie (in-process, eigen HKCU) zag 'm dan nooit. `ToElevatedRegPath` vertaalt naar de SID van de ingelogde gebruiker; `SnapshotService` gebruikt dezelfde vertaling bij restore én past de pad-regel toe op snapshots van vóór deze fix (die hebben de policy-entries nog met `RequiresElevation=false`).

**2. De Widgets-knop is géén UAC-probleem, en ook niet met UAC op te lossen.** Onderzoek (met verificatie op deze machine, probes hersteld): sinds 24H2 beschermt **UCPD.sys** — de "User Choice Protection Driver", een kernel-registry-filter — een vaste lijst waarden, waaronder `Explorer\Advanced\TaskbarDa` én de HKLM-policy `Dsh\AllowNewsAndInterests`. `SetValue` geeft `ACCESS_DENIED` voor elk proces dat niet door Microsoft gesigneerd is, en `reg.exe`, `powershell.exe`, `cmd.exe` en `regedit.exe` staan bovendien expliciet op de blokkeerlijst — ook elevated. `SetupToolbox.exe` is niet gesigneerd; de elevated batch gebruikt `powershell.exe` + `reg.exe`. Er is dus geen registry-route meer, en de "Group Policy workaround" die de oude omschrijving beloofde kan nooit komen. Win11Debloat (dec 2024, issue #162) en WinUtil (#2886) zijn om precies deze reden allebei overgestapt op het **verwijderen van het Widgets-pakket**. Dat doet deze versie ook:

- Nieuwe tweak-variant **`TweakPackageRemoval`** naast toggle en choice. Apply = `Remove-AppxPackage -User <SID>` voor `MicrosoftWindows.Client.WebExperience` (het Windows Web Experience Pack = Widgets) en `Microsoft.WidgetsPlatformRuntime`, na het stoppen van de Widgets-processen; **per gebruiker, bewust niet `-AllUsers` en niet deprovisioneren**, zodat de bestanden in `WindowsApps` blijven staan en de revert ze terug kan zetten. Is het pakket ook voor alle gebruikers weg, dan zegt de foutmelding (`tweak.package.noStaged`) dat het via de Store terug te halen is — we proberen géén `reg.exe`-kopie onder een andere naam of `sc config UCPD start=disabled`; dat werkt vandaag, maar is anti-tamper omzeilen en Microsoft A/B-test de tegenmaatregel al.
- **De revert draait bewust ONGE-ELEVEERD**, en dat is geen detail: `Add-AppxPackage -Register` kent geen `-User` (geverifieerd op de parametersets) en registreert dus altijd voor het account dat het aanroept. In een ge-eleveerd kindproces zou dat het admin-account zijn — precies dezelfde valkuil als bij de HKCU-policies, maar hier niet met een pad-truc op te lossen. Vanuit ons eigen asInvoker-proces klopt de context per definitie, en registreren van een al gestaged pakket voor jezelf vereist geen admin. Het manifest-pad komt uit `AppxAllUserStore\Applications` + `\InboxApplications`: `C:\Program Files\WindowsApps` is niet te doorzoeken, maar een pad dat je al kent is wél leesbaar, en die key noemt het manifest direct (bij een bundel het `AppxBundleManifest.xml`). Onge-eleveerd getest op de dev-machine.
- **Alleen het eerste pakket is bepalend.** `Microsoft.WidgetsPlatformRuntime` bestaat pas vanaf 24H2 en ontbreekt op 23H2 of op een image zonder Store. Telde zo'n nooit-geïnstalleerd pakket als "verwijderd" mee, dan kwam de tweak op `Partial` — en Partial telt in de UI als AAN, waardoor de schakelaar niet meer uit te zetten was en de revert bleef klagen over een pakket dat er nooit was. Nu bepaalt `PackageNames[0]` de status en zijn de rest best-effort: ontbreekt een secundair pakket bij de revert, dan gebeurt er niets in plaats van een fout.
- Detectie blijft in-process en snel: de per-gebruiker registratie onder `HKCU\Software\Classes\Local Settings\…\AppModel\Repository\Packages\<Name>_*`. Geen `Get-AppxPackage` bij page-load. De Dsh-policy (`AllowNewsAndInterests=0`, `DisableWidgetsBoard=1`) telt als alternate signal "al actief" — zo staat 'ie in de ISO van Windows11-Unattended-Debloat, en op de dev-machine geeft de kaart daarom *Actief*. De `PolicyManager\default\…\value`-key die er als alternate stond is weg: dat is het CSP-schema met stockwaarde 1, geen toestand.
- `RunElevatedBatchAsync` heet nu `RunBatchAsync(steps, elevated)` en neemt een lijst `BatchStep` (key, naam, script, ok-melding, foutvertaling) in plaats van registry-ops. Daardoor gaan reg.exe-stappen en pakketverwijdering in **dezelfde UAC-prompt** mee, en kan dezelfde machinerie de revert zónder prompt draaien. `ApplyChoiceAsync` is naar hetzelfde pad omgezet. Restart-eis van de tweak is `ExplorerRestart` zodat de knop direct verdwijnt; naam is *Widgets verwijderen (knop + paneel)* / *Remove Widgets (button + panel)*.

**3. De planning-dialog volgde het thema niet** — en geen enkele andere dialog ook. Bekend WinUI 3-gebrek (microsoft-ui-xaml #6577, open sinds 2022; #8077 loste het voor flyouts op in WinAppSDK 1.6, niet voor ContentDialog): een dialog met alleen een `XamlRoot` leeft in een losse popup onder de PopupRoot, en die krijgt de thema-walk van een Windows-themawissel tijdens runtime niet mee. Het venster kleurt live mee, elke dialog die je daarna opent staat nog in het thema van app-start. WinUI Gallery werkt er zelf omheen met `dialog.RequestedTheme = root.ActualTheme`. Dat doet `DialogService.ShowAsync` nu centraal, en hij volgt `ActualThemeChanged` zolang de dialog open staat (handler wordt in `finally` weer losgekoppeld). **Alle 17 directe `dialog.ShowAsync()`-aanroepen** (TweakApplyRunner, Debloat-, DeepClean-, Settings- en TweaksPage) gaan nu door `DialogService` — die hadden daarmee ook nog nooit de één-dialog-tegelijk-gate gehad waar die service voor bestaat.

**4. Wat de reviewronde erbovenop vond.** De patch is na afloop door vier review-lenzen gehaald (C#-correctheid, PowerShell/registry-semantiek, WinUI/UX, vertalingen) met een tegensprekende verificatie per bevinding. Zes punten waren echt en zijn meegefixt:

- **`reg.exe delete` van een waarde die er al niet was gold als fout.** De drie HKCU-policy-tweaks verhuisden naar de elevated batch, en die beoordeelde de exit-code — maar reg.exe geeft 1 zowel bij "bestond niet" als bij een echte fout. Wie de Office-bundel op de oude build had aangezet (12 writes geweigerd, MailSettings geslaagd → `Partial` → schakelaar AAN) kreeg 'm na de update niet uit zonder twaalf `reg.exe exit 1`-fouten. Nu wordt niet de exit-code maar het **eindresultaat** getoetst: staat de waarde er na afloop nog, dan pas is het fout. De in-process variant was daar altijd al tolerant in, en `SnapshotService` ook — de elevated batch was de enige die dat niet was.
- De `Partial`-val bij pakketverwijdering en de onge-eleveerde revert (beide hierboven beschreven).
- **Explorer werd ook herstart als de UAC-prompt geweigerd werd.** Sinds Widgets `ExplorerRestart` vraagt, betekende annuleren: bureaublad knippert, open vensters dicht, en er is niets veranderd. De herstart hangt nu aan dezelfde voorwaarde als de sign-out-melding (`SuccessCount > 0`).
- **`scan-untranslated.py` sloeg om van schoon naar exit 1** op het fragment `" -User '{sid}'"`. Terecht gezien de opzet — de bestaande regels dekken alleen fragmenten die een heel cmdlet noemen, en een literal in een variabele heeft geen aanroep-context. Categorisch opgelost in plaats van met een `ALLOW`-regel: een los `-Parameter`-token telt nu als PowerShell-argument. Getoetst tegen beide vertaaltabellen — van de 2712 UI-strings raakt het patroon er nul.
- **Kleinere zaken:** de foutmelding toonde `package\MicrosoftWindows.Client.WebExperience` (intern token in beeld; de result-key is aangepast zodat alleen de pakketnaam overblijft), `tweak.package.noStaged` had de Widgets-naam hardcoded terwijl de key generiek is (nu twee placeholders, met `StoreDisplayName` erbij op het model), de Widgets-omschrijving was vier zinnen mechanisme-uitleg en is ingekort, en twee sectie-commentaren in `TweakService` beweerden nog dat de HKCU-policy-keys zonder UAC gingen.

**Stringtabellen:** 1355 → **1356** keys (`tweak.package.noStaged`, met `{0}` voor de pakketnaam en `{1}` voor het Store-product-id); vijf bestaande waarden herschreven in beide talen.

> **Verificatie.** Build groen (0 warnings), `scan-untranslated.py` en `check-catalog-keys.py` allebei schoon. ACL-scan over alle 56 HKCU-paden in `TweakService` en de UCPD-probes zijn op de dev-machine gedaan (probes hersteld). Het gegenereerde PowerShell van álle nieuwe staptypen is door de parser gehaald (0 fouten) en de niet-destructieve delen zijn echt uitgevoerd: de staged-manifest-lookup werkt onge-eleveerd, en de delete-eindtoets slaagt terecht op een afwezige waarde en faalt terecht op een aanwezige.
>
> **De thema-fix is visueel bevestigd**, via het echte pad: app open in donker thema, Windows omgezet naar licht terwijl 'ie draaide, daarna de planning-dialog geopend — die komt nu licht op een lichte pagina. Vóór de fix was dat precies de donkere dialog uit de melding van user. Thema daarna teruggezet op donker. De Widgets-kaart is via UI Automation nagelopen en rendert met de nieuwe naam en omschrijving.
>
> **Niet gedaan, hoort in de test van user:** een echte apply/revert van de Widgets-tweak — op deze machine is het pakket al weg én staat de Dsh-policy uit de ISO, dus daar valt niets te toggelen. Wat getest moet worden: een machine mét Widgets, schakelaar aan → één UAC → Explorer-herstart → knop weg; schakelaar uit → pakket terug. En de Office-bundel op een machine mét Office, in beide richtingen (de revert-kant is de interessante: die moet nu zonder fouten door twaalf al-afwezige waarden heen lopen).
...
#### Echte schermafbeeldingen, met een NL/EN-schakelaar (2026-08-22)

De ontbrekende screenshots uit de vorige ronde zijn er. **Keuze van user: één galerij met een taalknop erboven**, zodat de pagina de tweetaligheid láát zien in plaats van 'm te claimen — dat was tot nu toe een bullet in een lijstje. De nagebouwde `AppPreview` in de hero is daarmee vervallen en verwijderd; die bestond alleen omdat er geen echte beelden waren.

**Twee rondes nodig geweest, en de eerste is een les.** De eerste set was schermvullend op 2560px geschoten. Nagemeten in de gebouwde site: het beeldkader is daar 958×577, dus schaal 0,60 — de 12px-tekst uit de app rendert op ~7px, en op een 360px-scherm op ~2,5px. Bovendien vulde de app-inhoud maar de bovenste eenderde en was de rest leeg venster. Onbruikbaar, en niet op te lossen met bijsnijden: onder die lege ruimte zit de betekenisvolle onderbalk (*"0 apps geselecteerd"* met de installeerknop), dus het midden wegknippen zou een naadje zijn en de onderbalk wegsnijden zou UI verwijderen die er echt staat. User heeft opnieuw geschoten met een venster van ~1720×1005. **Vuistregel voor een volgende keer: schiet UI-opnamen op de grootte waarop je ze toont, niet schermvullend.**

**Alle acht zijn pixelgelijk: 1600×935, samen 320 KB** (was 2,1 MB aan PNG's). Niet alleen binnen een taalpaar maar over alle vier de schermen heen — uitgesneden ten opzichte van het app-icoon in de titelbalk als anker, zodat er noch bij een taalwissel noch bij een schermwissel iets verspringt. Die maat staat op **één plek** in de code (`SHOT_SIZE` in `data/content.js`), met de verhouding eruit afgeleid; een volgende set moet dezelfde discipline aanhouden.

> **Wat de controle op de beelden opleverde, en dit is geen bijzaak.** Op vier van de tien kandidaten schemerde onderaan of opzij een ander venster door, en op één daarvan stond een **Claude Code-terminal met leesbare tekst** in beeld. Allemaal weggesneden. Twee kandidaten zijn afgevallen: één met gekleurde terminaluitvoer bovenin, en één waar *Tweaks* in de zijbalk een muis-hover had die de Nederlandse tegenhanger niet had — dat zou bij het wisselen van taal zichtbaar verspringen. Beide Debloat-lijsten staan ingeklapt, dus er is geen enkele app-naam te zien; user heeft bevestigd dat de tweak-tellers en het aantal van 86 apps zichtbaar mogen zijn.

**Zes reviewbevindingen verwerkt** op de galerij zelf: donkere omkadering in béíde thema's (alle beelden zijn van het donkere app-thema; in lichte modus stond er anders een zwart vlak in een witte kaart, en de plaatshouder klapte tijdens het laden van wit naar zwart), de sectie van `max-w-5xl` naar `max-w-6xl` gelijk aan zijn buren, het bijschrift op gereserveerde hoogte zodat een tabklik de pagina niet 23px op en neer duwt, een live region zodat een schermlezer de wissel ook hóórt, alle acht alt-teksten zelfstandig leesbaar gemaakt (de Engelse begonnen met *"Dezelfde …"* zonder referent) en met de trema's uit de app, en de actieve tab van 1,09:1 naar **6,40:1** contrast (licht) respectievelijk 8,38:1 (donker) — WCAG 1.4.11 vraagt 3:1.

Twee alt-details zijn ná de nieuwe uitsnede alsnog bijgesteld, want ze beschreven de óude beelden: de onderste tegelrij op Tweaks toont zijn tellers niet meer, en op Apps is de laatste tegel maar deels zichtbaar. Dat laatste blijft bewust zo — wegsnijden kan niet zonder de onderbalk mee te nemen, en met de zichtbare scrollbar leest het als *"de lijst gaat verder"*, wat klopt.

**Lockfile opgeschoond:** `package-lock.json` declareerde drie devDependencies die `package.json` niet kende (`typescript`, `@types/react`, `@types/react-dom`). Dat brak niets, maar de eerste de beste `npm install` zou de lock herschrijven en een ruisige diff geven op een onverwacht moment. 44 regels eruit, verder niets veranderd.
...
Drie keer dezelfde beweging: iets dat opgebouwd of ingelezen wordt en nergens landt, gaat eruit. Geen nieuwe keys — de tabellen blijven op **1347**.

#### De weergavelaag die niets weergaf

`DeepCleanDeleteResult.ResultsByPath` en `LeftoverDeleteResult.ResultsByPath` waren allebei een `Dictionary<string, (bool, string)>`. Geverifieerd met een grep over de héle `src`-boom (`.cs` én `.xaml`): **`ResultsByPath` komt uitsluitend voor binnen de twee servicebestanden zelf** — nul treffers in `Dialogs/`, nul in `Pages/`, nul in XAML. De vier consumenten (`DeepCleanDialog`, `LeftoverCleanupDialog`, `DebloatPage`, `DeepCleanPage`) lezen alleen `SuccessCount`, `FailedCount`, `BytesFreed` en `Cancelled`.

De message-helft is gesloopt; de dictionary blijft, want die draagt de per-pad-deduplicatie én de succes-telling (`results.Count(kv => kv.Value)`). Zinnen als *"Removed registry key"*, *"Deleted folder"* en *"Unknown type"* werden dus opgebouwd en verdwenen — en stonden met die reden in de `ALLOW`-lijst in plaats van vertaald te zijn. Dat hele **blok (a)** kon nu weg.

Het vierde veld in de `RESULT|pad|OK|reden`-regels van de ge-eleveerde PowerShell-batches blijft bewust wél staan: dat is een logfile en diagnostisch nuttig, geen UI. `Split('|', 4)` is ongewijzigd.

> De alternatieve uitweg — de dictionary alsnog ergens tonen — is afgevallen: dat is nieuwe UI plus 8 keys × 2 talen voor per-pad-informatie die je al kaart voor kaart gezien hebt.

#### De 24 subcategorie-omschrijvingen

**Keuze van user: weghalen.** Ze stonden in `apps.json`, werden netjes gedeserialiseerd naar `SubCategory.Description`, en `CategoryDetailPage` bindt alleen `Name` voor de groepsheader — dus geen mens heeft ze ooit gezien. Argument om ze niet alsnog te tonen: onder zo’n header staan kaarten die zélf al een omschrijving per app dragen, dus een tweede omschrijvingslaag maakt de pagina drukker zonder nieuwe informatie — en het zou 24 vertalingen kosten die verifieerbaar moeten blijven.

24 regels uit `apps.json`, het veld uit `AppModels.cs`. **`check-catalog-keys.py` hoefde niet mee**: dat script leidt de verwachte keyset af uit `apps.json` en vroeg alleen om `appSubcategory.<id>.name`, nooit om `.desc`. Voor en na de wijziging: **152 verwachte keys**, 10 categorieën, 24 subcategorieën, 110 app-records. Het commentaar op de plek van het veld beschrijft nu wat het zóu kosten om ze terug te halen (152 → 176 keys), zodat die keuze niet opnieuw uitgezocht hoeft te worden.

#### CONTRIBUTING.md — en dezelfde fout in README.md

Alle vijf de genoemde punten klopten nog: **.NET 8** (is 10), een **WPF**-structuur met `Views/` + `ViewModels/` + `Themes/` en een `Launcher/`-project (allemaal weg sinds v0.5.9), **MIT License** onderaan, `YOUR_USERNAME` als placeholder — dat bleken er **drie** in plaats van twee — en een "features die we zoeken"-lijst waarvan 8 van de 9 items allang gebouwd zijn.

Vier punten die er níet stonden en ook fout waren: *"Gebruik Material Design components"* (de app is Fluent, en `CLAUDE.md` verbiedt juist hardcoded kleuren), *"Comments: Engels voor code"* (de hele codebase heeft Nederlandse comments), pass 4 die als `Dialogs/`/`Pages/`/`Helpers/` beschreven staat terwijl `Services/` er sinds v1.2.8 bij hoort, en `cd src/SetupToolbox && dotnet run` in plaats van de werkende `dotnet run --project`-vorm.

De MIT-vermelding was de vervelendste, want die staat er als **juridische mededeling aan bijdragers** terwijl het project sinds v1.0.0 source-available en propriëtair is. Die stond ook in **`README.md:150`** (*"MIT — zie LICENSE."*) en is daar in dezelfde beweging rechtgezet — dat is geen aparte commit waard.

De herschreven versie beschrijft ook de vijfde scanner-pass uit v1.2.9.5 en noemt de juiste logmap.

> **Nieuw gevonden, op de lijst gezet:** de XML-docstring van `Diagnostics.Log` zegt dat er naar `%TEMP%` geschreven wordt, terwijl `Diagnostics.LogDir` sinds de introductie ervan `%LocalAppData%\SetupToolbox\logs` is. Alleen een comment, maar het is precies het soort ding dat een bijdrager op het verkeerde been zet — zie *klein onderhoud* hieronder.

> **Aangetoond (2026-08-21):**
> - Build: **0 errors, 0 warnings** op `1.2.9.6`.
> - `scan-untranslated.py`: **0 / 0 / 0 / 0 / 0**, en de `ALLOW`-lijst is opnieuw gekrompen door blok (a).
> - `check-catalog-keys.py`: **1347 = 1347**, 152 verwachte catalogus-keys — identiek aan vóór de wijziging, wat bewijst dat de 24 velden inderdaad buiten de keyset stonden.
> - `apps.json` is na de bewerking opnieuw geparsed: geldig JSON, 10 categorieën, 24 subcategorieën, 110 app-records — er is niets anders geraakt.
>
> **Nog te verifiëren door user:** een Deep clean- en een leftover-cleanup-batch, want die raken de gewijzigde delete-paden. De tellers op de resultaat-InfoBar moeten hetzelfde blijven doen.
...
Twee dingen die allebei hetzelfde patroon hadden: een onomkeerbare actie die zónder waarschuwing doorging. De stringtabellen gaan van 1338 naar **1345 keys** — elf erbij, vier eruit.

#### Een mislukt herstelpunt werd nergens gemeld

`DebloatPage` deed letterlijk `_ = await App.RestorePoint.CreateAsync(rpDescription);` — het resultaat werd weggegooid. Vier zinnen werden opgebouwd en verdwenen, en dat is erger dan cosmetisch: **een mislukt herstelpunt vóór een verwijder-batch werd nergens gemeld**, en dat is precies het vangnet waar het om gaat. Je zou het pas merken in de Systeemherstel-wizard, op het moment dat je ‘m nodig hebt.

Alle aanroepers nagelopen: `CreateAsync` heeft er **één**, `DebloatPage:574`. `DeepCleanDialog` en de bloatware-tak van `DebloatPage` roepen ‘m niet aan — die bundelen de checkpoint als PowerShell-regel in de ge-eleveerde batch, met een expliciete `catch {}` en de noot *"Niet fataal: delete gaat door."* Dat is een bewuste keuze en valt buiten dit item.

Nu twee dingen, want **keuze van user (2026-08-21): melden én vragen**:

- Een eigen `RestorePointResultBar` op de Debloat-pagina, groen bij succes en oranje bij mislukking met de reden erbij. Bewust een **eigen** bar en niet `CleanupResultBar`: die wordt ná de batch door de leftover-scan overschreven, en dan zou juist de waarschuwing verdwijnen. Succes wordt ook gemeld — anders is niet te zien dát het gebeurd is.
- Een blokkerende vraag *"Doorgaan zonder herstelpunt?"* vóórdat de verwijder-dialog opengaat. `DefaultButton` staat op `Close`: op een veiligheidsvraag hoort Enter te annuleren, niet door te drukken. Via `DialogService.ShowAsync` omdat WinUI per thread maar één `ContentDialog` open laat en de ge-eleveerde PowerShell er net een had.

Annuleren slaat alleen de **apps**-batch over; de bloatware-batch die er mogelijk al doorheen is en de leftover-scan erna lopen gewoon af. Daarom een `proceed`-vlag en geen `return` middenin de methode.

De vier zinnen zijn vertaald (`restorePoint.created`, `.err.processStart`, `.err.noOutput`, `.err.uacDenied`) en **blok (c) van de `ALLOW`-lijst** in `scan-untranslated.py` is weg: 73 → 70 uitzonderingen. De twee overige returns — de `reason` uit de PowerShell-output en `ex.Message` — blijven ruw, want dat is Windows’ eigen tekst en niet die van ons.

> Let op: *"Geen output van Checkpoint-Computer."* stond **niet** in blok (c). Die werd door `SCRIPT_OR_ARG` gefilterd omdat er een cmdlet-naam in staat — de "bekende prijs" die in de docstring van de scanner staat. Hij is nu alsnog vertaald, maar de scanner had ‘m nooit gemeld.

#### De losse Importeren-knop wiste je selectie zonder te vragen

`SettingsPage.ImportButton_Click` riep `ImportAsync(..., clearFirst: true)` aan en daar zat niets tussen: één misklik op *Importeren* plus een bestand kiezen en de selectie waar je een kwartier aan gewerkt had was weg, zonder undo. Bestond sinds v0.7.4 en was de enige destructieve knop in de app zónder confirm.

**Keuze van user: de knop gaat weg**, niet er een bevestiging omheen. Reden: sinds v1.2.9 herkent `ConfigBackupService.ReadAsync` een los `my-apps.json` al als *Vorm 2*, en `ConfigImportDialog` zet daar een voorbeeld-dialog omheen **inclusief** de overschrijf-waarschuwing (`OverwriteBar`, die opengaat zodra het app-vinkje aan staat). De twee knoppen deden dus hetzelfde met hetzelfde bestand — één met vangnet, één zonder. Twee knoppen die hetzelfde doen is verwarrender dan één.

Gevolgen: `SelectionImportExportService.ImportAsync` en het record `SelectionImportResult` zijn dood en verwijderd (`ExportAsync` blijft, die schrijft nog steeds een losse `my-apps.json`), en vier keys werden wees en zijn uit **beide** tabellen gehaald: `settings.selection.importFailed.title`, `.imported.title`, `.imported.body` en `.imported.bodySkipped`. `io.fileNotFound` / `io.jsonReadFailed` / `io.noAppsInFile` blijven staan — die worden ook door `ConfigBackupService` en `TweakProfileService` gebruikt, gecontroleerd. `settings.selection.desc` wijst nu door naar de Importeren-knop bij *Volledige configuratie*.

> **Aangetoond (2026-08-21):**
> - Build: **0 errors, 0 warnings** op `1.2.9.4`.
> - `scan-untranslated.py`: **0 / 0 / 0 / 0**, en de ALLOW-lijst is met drie regels gekrompen.
> - `check-catalog-keys.py`: **1345 = 1345**, geen wezen, geen lege waardes. "Gecontroleerd op aanroep" 619 → **626** — dat is +11 nieuw − 4 verwijderd, precies kloppend, dus er is geen dode key achtergebleven.
>
> **Nog te verifiëren door user:** de herstelpunt-InfoBar en de vraag daarna vergen een echte Debloat-batch met een falend herstelpunt — dat is niet iets om even na te bootsen. De verdwenen Importeren-knop is wel meteen te zien op de Instellingen-pagina.

### v1.2.9.3 — Drie zichtbare defecten met een bekende oorzaak

Klein onderhoud dat al een tijd op de lijst stond, gegroepeerd omdat het alle drie dingen zijn die je op een screenshot ziet en geen van drieën een keuze vraagt. Geen nieuwe keys — de tabellen blijven op **1338**.

**De filter-header in de Deep-clean-dialog telde het totaal.** Bij een actief filter stond er *"227 items gevonden — filter actief"* terwijl er 30 kaarten onder stonden. De oorzaak was volgorde, geen rekenfout: `HeaderText.Text` werd toegewezen **vóórdat** `filteredItems` bestond, dus er was op dat moment niets anders mee te geven dan `_items.Count`. De twee blokken zijn omgedraaid. De toewijzing staat nu bewust ook nog steeds vóór de lege-lijst-`return` — anders blijft de header op de telling van het vórige filter staan, en een stale getal is erger dan de lichte dubbeling met *"geen match voor …"* eronder.

**De download-regel op de install-kaart toonde de volledige URL.** *"Downloading https://github.com/HandBrake/…/HandBrake-1.11.2-x86_64-Win_GUI.exe"* liep over twee regels en liet de kaart verspringen. Nu alleen de bestandsnaam. Bewust **niet** met `Path.GetFileName`: die knipt op de laatste slash en houdt een eventuele `?query` of `#fragment` vast (*"setup.exe?token=abc"*), en op een URL zonder pad geeft ‘ie de host terug. `Uri.AbsolutePath` gooit query én fragment eraf, `UnescapeDataString` maakt `%20` weer een spatie. Parst het niet als absolute URI — winget kan ook een lokaal pad of een los woord loggen — dan blijft de ruwe tekst gewoon staan: zelfde regel als de mapping eromheen, wat we niet zeker weten vertalen we niet.

**`ParseSearchOutput` nam de Tag-kolom mee in het versienummer — en er zat een tweede bug in dezelfde twee regels.** Het bekende symptoom was cosmetisch: *"Beschikbaar via winget (v4.9.9   Tag: notepad)"*, omdat de versie-substring doorliep tot `sourcePos` en een tussengevoegde `Tag`-kolom dus meepakte. Opgelost door alleen het eerste token te nemen — een winget-versie bevat nooit een spatie. Bewust géén `IndexOf("Tag")`: de kolomkoppen zijn Engels op een Engelse Windows en die aanname wilden we hier niet nog een keer maken.

> **De tweede bug is niet cosmetisch.** `versionEnd` werd niet begrensd op de regellengte: `sourcePos > versionPos ? sourcePos : line.Length` levert bij een afgekapte regel een `versionEnd` vóórbij het einde, en `Substring` gooit dan een `ArgumentOutOfRangeException`. Die wordt in `SearchWingetRepoAsync` door de omhullende `catch` opgevangen, waarna **alle** zoekresultaten stil verdwijnen — geen melding, geen hits, niets. Eén rare regel in de winget-output kon dus de hele repo-zoekfunctie leegtrekken. Nu begrensd met `Math.Min`. Dit raakt meteen de nieuwe repo-zoekactie in de export-dialog uit v1.2.9.2, die op dezelfde parser leunt.

> **Aangetoond (2026-08-21):**
> - Build: **0 errors, 0 warnings** op `1.2.9.3`.
> - `scan-untranslated.py`: **0 / 0 / 0 / 0**. `check-catalog-keys.py`: **1338 = 1338**, 619 op aanroep gecontroleerd — ongewijzigd, want deze patch voegt geen keys toe en haalt er geen weg.
>
> **Nog te verifiëren door user:** de filter-header met een actief filter, en de download-regel tijdens een echte install.
