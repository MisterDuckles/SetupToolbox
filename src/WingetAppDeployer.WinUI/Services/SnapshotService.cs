...
//   %LOCALAPPDATA%\WingetAppDeployer.WinUI\snapshots\<id>.json
//
...
public sealed class SnapshotService
{
    ...
    public async Task<SnapshotMetadata?> CaptureAsync(IReadOnlyList<Tweak> tweaks, string description)
    {
        ...
        {
            Diagnostics.Log("WingetAppDeployer_snapshots.log", $"Capture failed: {ex.Message}");
            return null;
        }
    }
        ...
        RunElevatedRestoreAsync(IReadOnlyList<SnapshotEntry> entries)
    {
        var logPath = Path.Combine(Path.GetTempPath(),
            $"WingetAppDeployer_snapshot-restore_{DateTime.Now:yyyyMMdd_HHmmss}.log");
        ...
        var scriptPath = Path.Combine(Path.GetTempPath(),
            $"WingetAppDeployer_snapshot-restore_{DateTime.Now:yyyyMMdd_HHmmss}.ps1");
    ...
    public void Delete(string snapshotId)
    {
        ...
        {
            Diagnostics.Log("WingetAppDeployer_snapshots.log", $"Delete failed: {ex.Message}");
        }
    }
