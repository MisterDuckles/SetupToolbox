...
public sealed class RestorePointService
{
    ...
    public async Task<RestorePointResult> CreateAsync(string description)
    {
        var logPath = Path.Combine(Path.GetTempPath(),
            $"SetupToolbox_restorepoint_{DateTime.Now:yyyyMMdd_HHmmss}.log");
...
";
        ...
        var scriptPath = Path.Combine(Path.GetTempPath(),
            $"SetupToolbox_restorepoint_{DateTime.Now:yyyyMMdd_HHmmss}.ps1");
        ...
        {
            Diagnostics.Log("SetupToolbox_restorepoint.log", $"CreateAsync failed: {ex.Message}");
    ...
    {
        return $@"
# SetupToolbox auto-checkpoint — voor de delete-batch eronder.
try {{
